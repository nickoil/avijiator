...
| Tempo knob placement | Stays physically in the ARP section (rename target only: `&arpTempoBpm` → `&masterTempoBpm`, label unchanged). SEQUENCER's own Tempo knob is removed | Zero layout-budget rework — SEQUENCER is its own row, not width-budgeted, so losing a knob there doesn't touch row B's exact 1240px. **Flagged for reconsideration if it reads wrong once built** — this was the simplest placement, not an independently-argued UX call |
